...
| 9 | Etsy developer registration: Arman's action item, to be done before Step 2 build continues. **Approved and verified live 2026-08-21** — real API key confirmed working end-to-end (see decision 17). |
...
| 17 | **Etsy auth header fixed: `x-api-key` must be `keystring:sharedSecret`, not keystring alone** (2026-08-21). Etsy changed this Feb 9, 2026 — `real.ts` was written before a real key existed to test against and only sent the keystring, so it would have failed on first real use (`403: "Shared secret is required in x-api-key header."`). Reproduced live, fixed, reverified live (real Etsy search returned 2,342 total listings for "notion budget tracker", with genuine non-mocked `num_favorers`/`views`/`price` data). `RealEtsyDataSource` now takes both `apiKey` and `sharedSecret`; `getEtsyDataSource()` requires both env vars when in `real` mode. `num_favorers`/`views` field names confirmed unchanged from the original assumption — no further schema drift found. |

**Status: Step 2 is fully built (6/6 increments) and now verified against the real Etsy API, not just mock data (decisions 9, 17).** Mock remains the default (`ETSY_DATA_SOURCE=mock` in `.env`) pending a deliberate cutover — see the mock-to-real switch plan tracked separately. Google Trends remains a tracked future addition (decisions 10/12), not a blocker.
