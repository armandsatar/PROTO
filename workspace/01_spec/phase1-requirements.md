...
3. **Classification/synthesis only** ("is this Etsy listing an exact-angle match," "generate 3 title variants," combine sub-scores) — this is the only place the user's configured AI connector (BYOK Claude/OpenAI/Gemini/Grok, or the Groq/Llama default) is used. It takes the raw structured data pulled in 2.2 as input and returns structured JSON classifications, never raw prose scores.

**Groq/Llama default confirmed sufficient for build/smoke-testing:** since research never asks the AI connector to browse the web, plain Llama on Groq is fine as the zero-setup default for both production and Step 2 smoke-testing — Groq "Compound" is not needed. Section 5 of the source spec is accurate as written for Phase 1 scope.
...
| 14 | AI connector for Step 2 smoke-testing: Groq/Llama zero-setup default is sufficient, no BYOK key required to start. |
